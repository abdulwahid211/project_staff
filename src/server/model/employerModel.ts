...
export const CreateEmployer = async (object: Employer) => {
    const CreateQueryString = "INSERT INTO employer (Name, Email, Password, Address, City, Postcode) VALUES (?,?,?,?,?,?)"
    const SecuredPassword = await PasswordHash(object.Password)
    Db.query(
        CreateQueryString,
        [object.Name, object.Email, SecuredPassword, object.Address, object.City, object.Postcode], (err, results) => {
            if (err)
                console.log(err);
        }
    );
    ...
    const [row] = await promisePool.execute(FindEmployerQueryString, [object.Email]);
    return row[0].Email === object.Email
}

export const UpdateEmployer = async (object: Employer) => {
    const UpdateQueryString = "Update Employer set Name=?,Password=?,Email=?,Address=?, City=?, Postcode=? where Email=?;"
    const SecuredPassword = await PasswordHash(object.Password)
    Db.query(
        UpdateQueryString,
        [object.Name, SecuredPassword, object.Email, object.Address, object.City, object.Postcode, object.Email], (err, results) => {
            if (err)
                console.log(err);
        }
    );

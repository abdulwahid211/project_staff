...
export const CreateVacancies = async (object: any) => {
  const CreateQueryString =
    'INSERT INTO Vacancies (Title, Description, Created, EmployerID,Salary,Location,Contract, Sector) VALUES (?,?,?,?,?,?,?,?)';
  ...
  const [row] = await promisePool.execute(FindVacanciesQueryString, [
    object.Created,
    object.EmployerID,
  ]);
  if (row[0] == undefined) {
...
export const UpdateVacancies = async (object: any) => {
  const UpdateQueryString =
    'Update Vacancies set Title=?,Description=?,Created=?,EmployerID=?,Salary=?,Location=?,Contract=?, Sector=? where VacancyID=?;';
...
export const DeleteVacancies = async (VacancyID: number) => {
  ...
  Db.query(CreateQueryString, [VacancyID], (err, results) => {
    if (err) console.log(err);
  });
  const FindVacanciesQueryString = `
    SELECT * from Vacancies where VacancyID=?;`;
